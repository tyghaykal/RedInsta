...
function save_option($option_name, $option_value) 
{
    ...
    $_SESSION["OPTIONS"][$option_name] = $opt->get("option_value");
...
function get_option($option_name, $default_value = false)
{
    ...
    // Check if option exists in session already
    if (isset($_SESSION["OPTIONS"][$option_name])) {
        return $_SESSION["OPTIONS"][$option_name];
    }
